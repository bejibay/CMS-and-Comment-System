...
if(isset($data['email'])){
$result1 = $this->select("SELECT* FROM userdata WHERE email=:email",["email"=>$this->email]);
if(!empty($result1)){$verifiedpassword = password_verify($data['password'], $result1['password']);}
if($result1>0)$verifiedpassword = password_verify($data['password'], $result1['password']);
if(isset($result1['password'])){$verifiedpassword = password_verify($data['password'], $result1['password']);
    if($verifiedpassword)return $result1;
