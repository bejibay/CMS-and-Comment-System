...
$this->status =0;
if(isset($data['']isset($data['firstname']) && isset($data['lastname']) && isset($data['email']) && isset($data['password'])){
...
elseif(!$result1){
$result2 = $this->insert("INSERT INTO userdata(firstname,lastname,username,email,password,status, created,ipaddress)
VALUES(:firstname,:lastname,:username,:email,:password,:status,:created,:ipaddress)",[
"firstname"=>$this->firstname,"lastname"=>$this->lastname,"username"=>$this->username,"email"=>$this->email,
"password"=>$this->password,"status"=>$this->status,"created"=>$this->created,"ipaddress"=>$this->ipaddress]);
}
} 
...
if(!empty($result1['password'])){if(password_verify($data['password'], $result1['password']))
return $result1;var_dump($result1);}
else{return $result2 = "incorrect password details";}
}
